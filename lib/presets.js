...
/**
 * Compile a preset into { total, lead, segments, cues }.
 *
 * `countdown` adds three ticks before every segment change. `lead` is a
 * get-ready stretch before the run opens: it lives at negative times, so the
 * schedule itself — segments, cue times, total — is unchanged by it, and the
 * runtime simply starts the clock at `-lead` instead of 0.
 */
export function buildSchedule(preset, { countdown = false, lead = 0 } = {}) {
  ...
  // Get-ready ticks: the last three seconds of the lead-in, whatever its length.
  for (let n = Math.min(3, Math.floor(lead)); n >= 1; n -= 1) cues.push({ t: -n, type: 'tick' });
  cues.sort((a, b) => a.t - b.t);
  withHeadroom(cues);

  return { total, lead, segments, cues };
}
...
export function buildMeditationSchedule({ minutes, bellEvery = 0 }) {
  ...
  return {
    total,
    lead: 0, // the opening gong is the signal to settle; no get-ready countdown
